...
public class ScrollingUpgradesPanel implements ICompositeWidget {
    ...
    public ScrollingUpgradesPanel(List<Slot> slots, IUpgradeableObject upgradeableObject, WidgetContainer widgets,
        ...
        tooltips.add(0, GuiText.CompatibleUpgrades.text());
    ...
    private boolean singularitySlotHidden() {
        return !slots.isEmpty() && isDisabledSlotEmpty((AppEngSlot) slots.get(0))
    ...
    public void updateBeforeRender() {
        ...
        for (Slot s : slots) {
            ...
            if (!slots.isEmpty() && s == slots.get(0) && singularitySlotHidden()) {
                ((AppEngSlot) slots.get(0)).setSlotEnabled(false);
                continue;
            }
            ...
            setSlotPosition(slot, slotOriginX + 1, slotOriginY + 1);
